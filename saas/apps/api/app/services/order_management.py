...
Implements :class:`MockBrokerAPI` and :class:`OrderManager` for reliable order
execution. The module mirrors MQL4-style order operations with explicit error
    ...
    def send_order_reliable(
        ...
        for attempt in range(self.retry_attempts):
            try:
                ...
                if isinstance(exc, (TradeContextBusyError, InvalidPriceError)) and attempt < self.retry_attempts - 1:
                    exponential_backoff_sleep(
                        self.sleep_time * (2**attempt), self.sleep_maximum
                    )
                    continue
